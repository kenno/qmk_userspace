...
#    if !(defined(PALETTEFX_ENABLE_ALL_EFFECTS) || defined(PALETTEFX_GRADIENT_ENABLE) ||                             \
          defined(PALETTEFX_FLOW_ENABLE) || defined(PALETTEFX_RIPPLE_ENABLE) || defined(PALETTEFX_SPARKLE_ENABLE) || \
 ...
 *
 * @param palette A table of size 16, each entry being a 16-bit HSV color.
...
static bool PALETTEFX_FLOW(effect_params_t* params) {
    ...
    for (uint8_t i = led_min; i < led_max; ++i) {
        ...
        const uint8_t x1 =
            (uint8_t)((((int16_t)rot_c) * ((int16_t)x)) / 128) - (uint8_t)((((int16_t)rot_s) * ((int16_t)y)) / 128);
        const uint8_t y1 =
            (uint8_t)((((int16_t)rot_s) * ((int16_t)x)) / 128) + (uint8_t)((((int16_t)rot_c) * ((int16_t)y)) / 128);
...
static bool PALETTEFX_RIPPLE(effect_params_t* params) {
    ...
    if (params->iter == 0) {
        ...
        if (drops[drops_tail].amplitude == 0 && timer_expired32(g_rgb_timer, drop_timer)) {
        ...
        for (uint8_t j = 0; j < 3; ++j) {
            ...
            const uint16_t tick = scale16by8(g_rgb_timer - drops[j].time, 1 + rgb_matrix_config.speed / 4);
            if (tick < 4 * 255) {
    ...
    for (uint8_t i = led_min; i < led_max; ++i) {
        ...
        for (uint8_t j = 0; j < 3; ++j) {
            ...
            if (r_scaled < 255) {
                ...
                const uint8_t bump = scale8(ease8InOutApprox(255 - (uint8_t)r_scaled), drops[j].amplitude);
        ...
        rgb_t rgb = rgb_matrix_hsv_to_rgb(palettefx_interp_color(palette, (uint8_t)value));
        rgb_matrix_set_color(i, rgb.r, rgb.g, rgb.b);
    }
...
static bool PALETTEFX_SPARKLE(effect_params_t* params) {
    RGB_MATRIX_USE_LIMITS(led_min, led_max);
    const uint16_t* palette    = palettefx_get_palette_data();
    const uint8_t   time       = palettefx_scaled_time(g_rgb_timer, 1 + rgb_matrix_config.speed / 8);
...
static bool PALETTEFX_VORTEX(effect_params_t* params) {
    RGB_MATRIX_USE_LIMITS(led_min, led_max);
    const uint16_t* palette = palettefx_get_palette_data();
    const uint16_t  time    = palettefx_scaled_time(g_rgb_timer, 1 + rgb_matrix_config.speed / 4);
...
static bool PALETTEFX_REACTIVE(effect_params_t* params) {
    ...
    for (uint8_t j = 0; j < count; ++j) {
        const uint16_t tick = scale16by8(g_last_hit_tracker.tick[j], 1 + rgb_matrix_config.speed / 4);
        if (tick <= 255) {
...
_Static_assert(NUM_PALETTEFX_PALETTES >= 1,
               "palettefx: No palettes are enabled. To fix: enable all built-in palettes by adding in config.h "
               "`#define PALETTE_ENABLE_ALL_PALETTES`, or enable at least one with `#define PALETTE_<name>_ENABLE`, or "
               "define at least one custom palette in palettefx_user.inc.");
_Static_assert(NUM_PALETTEFX_PALETTES <= 256 / RGB_MATRIX_HUE_STEP,
               "palettefx: Too many palettes. Up to 32 (= 256 / RGB_MATRIX_HUE_STEP) palettes are supported. "
               "Otherwise, some palettes would be unreachable.");

/** Gets the index of the selected palette. */
static uint8_t palettefx_get_palette(void) {
    uint8_t i = (rgb_matrix_get_hue() / RGB_MATRIX_HUE_STEP) % NUM_PALETTEFX_PALETTES;
        ...
        if (hsv.h >= 256 - 2 * RGB_MATRIX_HUE_STEP) {
            if (hsv.h >= 256 - RGB_MATRIX_HUE_STEP) {
                ...
                hsv.h = RGB_MATRIX_HUE_STEP *
                        ((256 / (RGB_MATRIX_HUE_STEP * NUM_PALETTEFX_PALETTES)) * NUM_PALETTEFX_PALETTES - 1);
            } else {
